var utilities = require('./Huawei_utils');
...
API.getMonthStatistics = function(token, callback) {
  ...
  utilities.contactRouter(uri, token, null, function(error, responseBody, response) {
...
API.multiBasicSettings = function (token, callback) {
    var uri = url('http://', this.options.gateway, '/api/wlan/multi-basic-settings');
    utilities.contactRouter(uri, token, null, function (error, responseBody, response) {
...
API.getToken = function (callback) {
  ...
  utilities.contactRouter(uri, {}, null, function(error, responseBody, response) {
    callback(error, {
...
API.getSignalStatus = function (token, callback) {
    var uri = url('http://', this.options.gateway, '/api/device/signal');
    utilities.contactRouter(uri, token, null, function (error, responseBody, response) {
        callback(error, response);
    });
...
API.setLedOn = function (token, value, callback) {
  ...
  utilities.contactRouter(uri, token, body, function(error, responseBody, response) {
    callback(error, response);
  });
};

API.setWifi = function(token, body, callback) {
    var uri = url('http://', this.options.gateway, '/api/wlan/multi-basic-settings');
    utilities.contactRouter(uri, token, body, function(error, responseBody, response) {
        callback(error, responseBody, response);
    });
...
API.reboot = function (token, callback) {
    var uri = url('http://', this.options.gateway, '/api/device/control');
    var body = '<?xml version="1.0" encoding="UTF-8"?><request><Control>1</Control></request>'
    utilities.contactRouter(uri, token, body, function (error, responseBody, response) {
        callback(error, responseBody, response);
    });
};
