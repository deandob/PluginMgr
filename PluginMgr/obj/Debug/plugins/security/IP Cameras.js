            ...
            for ( var i = 0; i < files.length; ++i ) { 
                ...
                fs.stat( fw.settings.cachepath + "/" + files[i], function ( err, stat ) {
                    ...
                    if ( fileCnt === files.length ) {           // got all the directory info async
                        ...
                        if ( newFiles.length > 0 ) fw.toHost( cameras[mycam].name, "recfiles", JSON.stringify( newFiles ) )          // send new recorded files to any active subscribers
...
function startup() {
    ...
    if ( startCameras() ) {
        ...
        if ( liveServer.listen( parseInt(fw.settings.liveport ) ) ) {
            fw.log( "Live video server running at port " + fw.settings.liveport );
        } else {
            fw.log( "Unable to start live video server on port " + fw.settings.liveport );
        }

        // Recorded video Server
        recServer = http.createServer( recStream );
        if ( recServer.listen( parseInt( fw.settings.recport ) ) ) {
            fw.log( "Recording video server running at port " + fw.settings.recport );
        } else {
            fw.log( "Unable to start recording video server on port " + fw.settings.recport );
        }
    ...
    switch ( scope ) {
        ...
        case "recfiles":
            for ( var myCh in fw.channels ) if ( fw.channels[myCh].name === channel ) {                   // find the camera in the camera array
                ...
                fw.toHost( channel, "recfiles", JSON.stringify( files ) )
                return "OK";
            }
            return;
            break;
